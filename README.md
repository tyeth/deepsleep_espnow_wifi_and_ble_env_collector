...
| `certs/fullchain.pem`, `certs/key.pem` | SD (`/sd/certs`) first, then flash `/certs` | HTTPS certificate for `192dot168dot4dot1.gundryconsultancy.com`; renewable via the page's *sync cert* (direct fetch, falling back through a chain of CORS proxies) / *upload* (both files, or one at a time) or auto when online |
